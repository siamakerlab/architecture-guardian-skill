...
5. Use available static search and dependency/code graph tools to find coupling, state ownership, and boundary drift; verify tool findings against source before reporting.
6. REVIEW, CHANGE-REVIEW, REFACTOR, and FULL-AUDIT outputs must include evidence-based Scorecard with `0-5` or `N-A`.
7. Keep output focused on required architecture judgment: impact, scorecard, findings, required tests, and verdict.
