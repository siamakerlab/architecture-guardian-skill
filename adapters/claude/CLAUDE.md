...
6. Use available static search and dependency/code graph tools to find coupling, state ownership, and boundary drift; verify tool findings against source before reporting.
7. Every report must follow `core/reports/ARCHITECTURE_GUARDIAN_REPORT.md`.
8. REVIEW, CHANGE-REVIEW, REFACTOR, and FULL-AUDIT outputs must include evidence-based Scorecard with `0-5` or `N-A`.
9. Keep output focused on required architecture judgment: impact, scorecard, findings, required tests, and verdict.
